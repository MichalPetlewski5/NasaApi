  ...
  return (
    <div className='p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap'>
        <div className='flex flex-col sm:flex-row sm:item-center sm-flex-wrap gap-x-4'>
            <h4 className="text-3xl hidden sm:inline sm:text-4xl md:ext-5xl font-semibold text-slate-400">
                0{index + 1}
            </h4>
            <h2 className='capitalize whitespace-nowrap truncate max-w-full text-lg sm:text-xl mm:text-2xl flex-1 sd:text-center '>{exercise.name.replaceAll('_', ' ')}</h2>
            <p className='text-sm text-slate-400 capitalize'>{exercise.type}</p>
        </div>
        <div className='flex flex-col'>
            <h3 text-slate-400 text-sm>Muscle Groups</h3>
            <p className='capitalize'>{exercise.muscles.join(' & ')}</p>
        </div>
        <div className='grid :grid-cols sm:grid-cols-4 sm:place-items-center gap-2'>
            ...
            <button onClick={handleSetIncrement} className='flex flex-col p-2 rounded border-[1.55px] duration-200 border-solid border-blue-900 hover:border-blue-600 w-full'>
