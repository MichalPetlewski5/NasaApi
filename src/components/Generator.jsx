  ...
  return(
    <div className='flex flex-col gap-4'>
      <div className='flex items-center gap-2 justify-center'>
        ...
        <h4 className='text-xl sm:text-2xl md:text-3x'>{title}</h4>
      </div>
  ...
  return (
    <SectionWrapper id={'generate'} header={"generate your workout"} title={['It\'s', 'Huge', 'o\'Clock']}>
        ...
        {Object.keys(WORKOUTS).map((type, typeIndex) =>{
          return(
            <button onClick={() => {
              setMuscles([])
              setPoison(type)
            }} className={'bg-slate-950 border px-4  py-4 rounded-lg duration-200 hover:border-blue-600 '
      ...
      <div className='bg-slate-950  border border-solid border-blue-400 rounded-lg flex flex-col'>
        <button onClick={handleDropDown} className='relative p-3 flex justify-center'>
      ...
      <Button updateWorkout={updateWorkout} text={'Formulate'}/>
    </SectionWrapper>
  )
}
